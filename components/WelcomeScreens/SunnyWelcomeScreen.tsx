...
const SunnyWelcomeScreen = () => {
  ...
  const movingText = useAnimatedStyle(() => ({
   ...
   const movingBlik = useAnimatedStyle(() => ({
    opacity: sunOpacity.value,
    transform: [{rotate: `${blikRotation.value * 360}deg`}, {scale: sunOpacity.value}]
  }))
  ...
  React.useEffect(() => {
    ...
    blikRotation.value = withRepeat(withTiming(1, {duration:5000, easing:Easing.linear}), 0)
  }, [])
  return (
    <View style={styles.mainContainer}>
      <ImageBackground source={require('../../assets/sunnybg.jpg')} style={styles.background}>
      <Animated.View style={[movingSun]}>
        <Image style={styles.sun} source={require('../../assets/sun2.png')}/>
      </Animated.View>
      <Animated.View style={[movingBlik]}>
        <Image style={styles.blik} source={require('../../assets/blik.png')}/>
      </Animated.View>
